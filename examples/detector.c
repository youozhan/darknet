...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while(1){
        ...
        //save without image itself
        image black_bg = load_image_color("/Users/yz/Documents/YOLO/video/black.png", im.w, im.h);
        draw_detections(black_bg, l.w*l.h*l.n, thresh, boxes, probs, masks, names, alphabet, l.classes);
        if(outfile){
            save_image(black_bg, outfile);
        }
        else{
            save_image(black_bg, "predictions");
#ifdef OPENCV
